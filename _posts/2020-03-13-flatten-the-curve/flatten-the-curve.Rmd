...
To build the model, I am going to take a repoductive rate of 2.28.
Additionally, we can add some noise to it in order to simulate 1,000 different scenarios.
...
Normal might not be the best probability distribution to assume for this number (here's where a [split or two piece distribution](https://en.wikipedia.org/wiki/Split_normal_distribution) would make sense because we know it might not be much lower but could be much higher.)
...
Now when we talk about flattening the curve, if we can reduce the repoductive rate mean value from above 2 to one, we can see the following:
...
Regardless, we are still looking at a lot of people sick.
The entire county could quite possibly be infected, but at least there is some delay. 
...
All in that gives up `r 921 + 885` beds.
...
Now we can simulate the rough cenus or volume of patients in the hospital at any given time using the following simulation.
...
If we can lower the repductive rate through the normal pathways: hand washing, social distancing, etc, we can see we will still have an issue:
...
We need to be cautios and exericse care when dealing with these kinds of things and do our part to "flatten the curve".